...
/* Author : Igor Koz mwSize  sev   igor@ifp.uiuc.edu   
   ...
   G is stored as array of doubles in Matlab which is very inefficient.   
    mwSize  ernal representation in this programm is unsigned char. Please modify   
 ...
 *          are supplied.   
 */    
const  mwSize   log4[4]   = {0,0,1,2}; /* stores i at address \alpha^i */    
const  mwSize   log8[8]   = {0,0,1,3,2,6,4,5};    
const  mwSize   log16[16] = {0,0,1,4,2,8,5,10,3,14,9,7,6,13,11,12};    
const  mwSize   log32[32] = {0,0,1,18,2,5,19,11,3,29,6,27,20,8,12,23,4,    
            10,30,17,7,22,28,26,21,25,9,16,13,14,24,15};    
const  mwSize   log64[64] = {0,0,1,6,2,12,7,26,3,32,13,35,8,48,27,18,4,24,   
...
const  mwSize   log128[128] = {0,0,1,31,2,62,32,103,3,7,63,15,33,84,104,    
...
const  mwSize   log256[256] = {0,0,1,25,2,50,26,198,3,223,51,238,27,104,199,75,4,100,   
...
const  mwSize   exp4[3]   = {1,2,3}; /* stores \alpha^i at address i */    
const  mwSize   exp8[7]   = {1,2,4,3,6,7,5};    
const  mwSize   exp16[15] = {1,2,4,8,3,6,12,11,5,10,7,14,15,13,9};    
const  mwSize   exp32[31] = {1,2,4,8,16,5,10,20,13,26,17,7,14,28,29,31,    
            27,19,3,6,12,24,21,15,30,25,23,11,22,9,18};    
const  mwSize   exp64[63] = {1,2,4,8,16,32,3,6,12,24,48,35,5,10,20,40,19,  
...
const  mwSize   exp128[127] = {1,2,4,8,16,32,64,9,18,36,72,25,50,100,65,11,
...
const  mwSize   exp256[255] = {1,2,4,8,16,32,64,128,29,58,116,232,205,135,19,38,76,    
...
/* For testing:   
   ...
   while(1){   
   pr mwSize  f("please enter a b q:");   
   scanf("%u %u %u",&i,&j,&q);   
   pr mwSize  f("\n a * b in GF(q) = %u\n",GFq_m(i,j,q));   
   }   
   }   
...
{    
  unsigned char **HH, **GG;    
   mwSize   ii, jj, *ir, *jc, rdep, tmp, d, q, scale;    
  double *sr1, *sr2, *g;    
   mwSize   N,M,K,i,j,k,kk,nz,*irs1,*jcs1, *irs2, *jcs2;    
...
/* invert HH matrix here */    
    ...
    ir = ( mwSize   *)mxMalloc(M*sizeof( mwSize  ));    
        jc = ( mwSize   *)mxMalloc(N*sizeof( mwSize  ));    
    for( i=0 ; i<M ; i++)    
        ir[i] = i;    
    for( j=0 ; j<N ; j++)    
        jc[j] = j;    
...
/* NOTE, it is a very inefficient way to store G. Change to taste!*/    
    ...
    /* Assign po mwSize  ers to the output matrix */    
    g = mxGetPr(G_OUT);    
    for(i=0 ; i<K ; i++)    
        for(j=0 ; j<N; j++)    
            g[i+j*K] = GG[i][j];    
